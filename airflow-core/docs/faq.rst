...
Why do ``logical_date`` and ``data_interval_start`` differ for some manually triggered DAG runs?
-----------------------------------------------------------------------------------------------

In Airflow 3, manually triggered DAG runs should not assume that ``data_interval_start`` is derived
from, or equal to, the supplied ``logical_date``.

For scheduled runs, these values are closely tied to the timetable. For manually triggered runs,
the resulting ``data_interval`` depends on the timetable and the trigger path, and some APIs also
allow the interval to be provided explicitly.

If your DAG logic needs the user-specified date for a manual run, use ``logical_date`` explicitly.

For detailed migration guidance, see :ref:`data-interval-manual-triggering`.

How should I adapt DAG code that used ``data_interval_start`` for manual triggering?
------------------------------------------------------------------------------------

If you used ``data_interval_start`` to represent the requested manual run date, update that logic to
use ``logical_date`` instead.
...
Keep using ``data_interval_start`` and ``data_interval_end`` when you specifically need the run's
resolved interval semantics rather than the user-supplied date.
